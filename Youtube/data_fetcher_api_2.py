    ...
    def fetch_data(self):
        for query in self.queries:
            ...
            for video in self.video_details:
                ...
                channel_videos[channel_id].append(video_data)

            # Filter channels by view count of the latest 2 videos
            eligible_channels = {}
            for channel_id, videos in channel_videos.items():
                # Sort videos by view count in descending order
                videos.sort(key=lambda x: x['view_count'], reverse=True)
                # Check if the two latest videos have met the minimum view count requirement
                if len(videos) >= 2 and all(video['view_count'] >= self.min_view_count for video in videos[:2]):
                    eligible_channels[channel_id] = videos[:2]  # Only store the two latest videos if they meet criteria

            # Get channel information for eligible channels
            self.channel_info = self.get_channels_about(list(eligible_channels.keys()))
            
            for channel_id, videos in eligible_channels.items():
                ...
                if channel_name not in self.existing_channel_names:
                    ...
                    for video in videos:
                        self.data_to_store.append({
                            "video_id": video['video_id'],
                            "video_description": self.video_descs.get(video['video_id'], "No description available"),
